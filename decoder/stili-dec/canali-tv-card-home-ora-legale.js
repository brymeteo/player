...
// Funzione che determina se siamo in ora legale (DST) secondo la regola EU
function isDST(date) {
  const year = date.getFullYear();
  // Calcola l'ultima domenica di marzo
  const lastMarch = new Date(year, 2, 31);
  while (lastMarch.getDay() !== 0) {
    lastMarch.setDate(lastMarch.getDate() - 1);
  }
  // Calcola l'ultima domenica di ottobre
  const lastOctober = new Date(year, 9, 31);
  while (lastOctober.getDay() !== 0) {
    lastOctober.setDate(lastOctober.getDate() - 1);
  }
  return date >= lastMarch && date < lastOctober;
}

// Funzione per aggiustare l'orario dei programmi
function adjustProgramTime(date) {
  // Utilizza la data corrente per determinare lo stato DST
  if (isDST(new Date())) {
    // Ora legale: aumenta di 1 ora
    date.setHours(date.getHours() - 1);
  } else {
    // Ora solare: sottrai di 1 ora
    date.setHours(date.getHours() - 1);
  }
  return date;
}
...
async function fetchPrograms() {
  ...
  channels.forEach((channel) => {
    const currentProgram = channel.programs.find(p => {
      // Applica l'aggiustamento agli orari
      const start = adjustProgramTime(new Date(p.start));
      const end = adjustProgramTime(new Date(p.end));
      return now >= start && now <= end;
    });

    if (currentProgram) {
      // Applica l'aggiustamento agli orari del programma
      const startTime = adjustProgramTime(new Date(currentProgram.start));
      const endTime = adjustProgramTime(new Date(currentProgram.end));
...
function initialize() {
  ...
  // Aggiorna la barra di progresso ogni 20 secondi
